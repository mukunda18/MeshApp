...
object HeaderProtocol {
    ...
    private const val IMMEDIATE_SENDER_NODE_ID_OFFSET = RESERVED_OFFSET + RESERVED_LENGTH
    private const val IMMEDIATE_SENDER_NODE_ID_LENGTH = 32

    private const val SOURCE_NODE_ID_OFFSET = IMMEDIATE_SENDER_NODE_ID_OFFSET + IMMEDIATE_SENDER_NODE_ID_LENGTH
    ...
    object ImmediateSenderNodeId : Field<NodeId> {
        override fun read(data: ByteArray, baseOffset: Int): ReadWithLength<NodeId> =
            ReadWithLength(
                NodeId(readBytes(data, baseOffset + IMMEDIATE_SENDER_NODE_ID_OFFSET, IMMEDIATE_SENDER_NODE_ID_LENGTH)),
                IMMEDIATE_SENDER_NODE_ID_LENGTH
            )

        override fun write(data: ByteArray, value: NodeId, baseOffset: Int): Int {
            writeBytes(data, baseOffset + IMMEDIATE_SENDER_NODE_ID_OFFSET, value.bytes, IMMEDIATE_SENDER_NODE_ID_LENGTH)
            return IMMEDIATE_SENDER_NODE_ID_LENGTH
        }
    }
    ...
    object Id : Field<MessageId> {
        override fun read(data: ByteArray, baseOffset: Int): ReadWithLength<MessageId> =
            ReadWithLength(MessageId(readBytes(data, baseOffset + ID_OFFSET, ID_LENGTH)), ID_LENGTH)

        override fun write(data: ByteArray, value: MessageId, baseOffset: Int): Int {
            writeBytes(data, baseOffset + ID_OFFSET, value.bytes, ID_LENGTH)
            return ID_LENGTH
        }
    }
