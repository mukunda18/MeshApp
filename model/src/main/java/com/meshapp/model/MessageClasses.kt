...
data class MessageId(val bytes: ByteArray) {
    init {
        require(bytes.size == 8) { "MessageId must be exactly 8 bytes, got ${bytes.size}" }
    }
...
fun randomMessageId(): MessageId {
    val msb = java.util.UUID.randomUUID().mostSignificantBits
    val bytes = ByteArray(8)
    for (i in 0..7) bytes[i] = (msb shr (56 - i * 8)).toByte()
...
data class Header(
    ...
    val ttl: Int,
    val reserved: Int,
    val immediateSenderNodeId: NodeId,
