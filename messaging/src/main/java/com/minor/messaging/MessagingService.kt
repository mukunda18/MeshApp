...
class MessagingService(
    ...
    private val meshGateway: MeshService,
    ...
    fun start() {
        ...
        scope.launch {
            meshGateway.incomingMessageStream.collect { (sourceNodeId, payload) ->
    ...
    fun send(destinationNodeID: NodeId, plaintext: String): Message {
        ...
        val envelopeBytes = securityCodec.encode(plaintext, destinationNodeID, messageId)
        val envelope = parseEnvelope(envelopeBytes)

        meshGateway.sendMessage(destinationNodeID, Payload.Message(envelope), messageId)
    ...
    private fun handleIncomingMessage(sourceNodeId: NodeId, payload: Payload.Message) {
        try {
            val decoded = securityCodec.decode(serializeEnvelope(payload.envelope))
            val message = Message(
